context('ui', () => {
  ...
  it('navigates by stock arrows', () => {
    cy.get('[data-cy=arrows]').within(() => {
      ...
      cy.get('.ssr-carousel-next-button')
        .click()
        .slideHidden(1)
        .slideVisible(2);
      cy.get('.ssr-carousel-back-button')
        .click()
        .slideHidden(2)
        .slideVisible(1);
    });
  ...
  it('navigates by dot', () => {
    cy.get('[data-cy=dots]').within(() => {
      cy.wait(500);
      // Click on page 2 dot
      cy.get('.ssr-carousel-dot-button[aria-label="Page 2"]')
        .click()
        .slideHidden(1)
        .slideVisible(2);
    });
  ...
  it('can disable dragging', () => {
    cy.get('[data-cy=no-drag]').within(() => {
      ...
      cy.dragLeft().slideVisible(1).slideHidden(2).slideHidden(3);
